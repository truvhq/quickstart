...
app.get('/api/orders', (_req, res) => {
  try {
    ...
    res.json(orders.map(o => ({
      order_id: o.id, truv_order_id: o.truv_order_id, demo_id: o.demo_id,
      status: o.status, share_url: o.share_url, created_at: o.created_at,
      raw_response: o.raw_response ? JSON.parse(o.raw_response) : {},
    })));
  } catch (err) { console.error('GET /api/orders error:', err); res.status(500).json({ error: 'Internal server error' }); }
});
...
app.post('/api/orders', async (req, res) => {
  try {
    ...
    db.createOrder({
      ...
      demoId: 'employee-portal', bridgeToken: truvData.bridge_token,
    ...
    res.json({ order_id: orderId, truv_order_id: truvData.id, share_url: truvData.share_url, status: truvData.status });
...
app.get('/api/orders/:id', async (req, res) => {
  try {
    ...
    res.json({ order_id: order.id, truv_order_id: order.truv_order_id, status: order.status, share_url: order.share_url, raw_response: raw });
